...
export function QuizProgress({ state }: QuizProgressProps) {
  ...
  return (
    <div className="flex items-center gap-5 text-sm" role="status" aria-live="polite" aria-label={`Streak: ${state.currentStreak}. Score: ${state.totalCorrect} of ${state.totalAnswered}. Cards: ${collected} of ${total}.`}>
      {/* Streak */}
